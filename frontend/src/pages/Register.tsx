  ...
  return (
    <div className="min-h-screen flex flex-col bg-black relative">
      ...
      <main className="flex-grow flex items-center justify-center z-10 p-4">
        <div className="bg-black/80 p-12 rounded-lg w-full max-w-md shadow-2xl mt-12">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              <input 
                ...
                className="w-full bg-[#333] text-white p-4 rounded focus:outline-none focus:bg-[#454545]"
                {...registerForm('name', { required: 'Nome é obrigatório' })}
              />
              {errors.name && <p className="text-netflix-red text-xs mt-2">{(errors.name as any).message}</p>}
            </div>
            ...
            <div>
              <input 
                ...
                className="w-full bg-[#333] text-white p-4 rounded focus:outline-none focus:bg-[#454545]"
                {...registerForm('password', { required: 'Senha é obrigatória' })}
              />
          ...
          <div className="mt-12 text-[#8c8c8c]">
            Já tem uma conta? <Link to="/login" className="text-white hover:underline">Entre aqui.</Link>
          </div>
        </div>
      </main>
    </div>
